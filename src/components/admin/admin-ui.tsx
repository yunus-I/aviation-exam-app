...
export function Button({ variant = "primary", size = "md", loading, children, className = "", disabled, ...props }: BtnProps) {
 ...
 primary: "admin-btn-primary shadow-sm hover:shadow-md active:scale-[0.98]",
 secondary: "bg-white text-[#1A202C] border border-[#E4E8F0] hover:bg-[#F7F8FC] hover:border-[#CBD5E1] active:scale-[0.98]",
 danger: "bg-white text-red-600 border border-red-200 hover:bg-red-50 hover:border-red-300 active:scale-[0.98]",
 ghost: "bg-transparent text-[#64748B] hover:bg-[#F7F8FC] hover:text-[#1A202C]",
 };
...
export function StatCard({ label, value, icon, trend, trendUp }: StatCardProps) {
 ...
 <div className="w-10 h-10 rounded-xl admin-icon-wrap flex items-center justify-center text-[#003580] flex-shrink-0">
 {icon}
 </div>
...
export function Input({ label, error, icon, className = "", ...props }: InputProps) {
 const base = "w-full px-3.5 py-2.5 bg-white border border-[#E4E8F0] rounded-xl text-sm text-[#1A202C] placeholder:text-[#94A3B8] focus:border-brand focus:ring-2 /10 outline-none transition";
 return (
...
export function Select({ label, className = "", children, ...props }: SelectProps) {
 ...
 <div className="flex flex-col gap-1.5">
 {label && <label className="text-xs font-semibold text-[#1A202C]">{label}</label>}
 <select className={`w-full px-3.5 py-2.5 bg-white border border-[#E4E8F0] rounded-xl text-sm text-[#1A202C] focus:border-brand focus:ring-2 /10 outline-none transition ${className}`} {...props}>
 {children}
 </select>
 </div>
 );
...
export function SectionHeader({ title, description, icon, action }: SectionHeaderProps) {
 ...
 {icon && <div className="w-8 h-8 rounded-lg admin-icon-wrap flex items-center justify-center text-[#003580] flex-shrink-0">{icon}</div>}
 <div>
